        ...
        for example in train_examples:
            ...
            if len(lm_texts) >= nll_batch_size and want_nll_step():
                ...
                if global_step % grad_accum == 0:
                    ...
                    if reference is None and optimizer_steps_total >= nll_warmup_steps:
                        reference = AutoModelForCausalLM.from_config(policy.config).to(device)
                        reference.load_state_dict(policy.state_dict())
                        reference.eval()
                        for p in reference.parameters():
                            p.requires_grad_(False)
                        print(f"[Model] Reference snapshot created at optimizer_step={optimizer_steps_total}")
                    ...
                    if optimizer_steps % save_interval == 0:
                        ...
                        if wandb_project is not None:
                            wandb.log({"checkpoint_step": optimizer_steps, "global_step": global_step})
            ...
            if len(prompts) == batch_size:
                if not want_dpo_step():
                    # Skip doing DPO this time; clear buffers and keep collecting.
                    prompts, chosen, rejected = [], [], []
                    continue
                ...
                if reference is None:
                    ...
                    for p in reference.parameters():
                        p.requires_grad_(False)
                    print("[Model] Reference snapshot created (fallback).")
...
def parse_args():
    ...
    parser.add_argument("--nll_steps_per_cycle", type=int, default=4)
